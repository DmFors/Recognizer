    ...
    for lexeme in chain_lexemes:
        ...
        if state == "НАЧ":
            if word == "type":
                ...
                raise SyntaxUnitError(f"Expected 'type', but received '{word}' on state {state}")
        elif state == "TYPE":
            if word_class == "идент" and identify_pascal(word) is False:
                state = "ИМЯ_МАССИВА"
            else:
                raise IdUnitError(f"Invalid array name on state {state}")
        elif state == "ИМЯ_МАССИВА":
            if word == "=":
                state = "РАВНО"
            else:
                raise SyntaxUnitError(f"Excepted '=', but received {word} on state {state}")
        elif state == "РАВНО":
            if word == "array":
                state = "ARRAY"
            else:
                raise SyntaxUnitError(f"Expected 'array', but received '{word}' on state {state}")
        elif state == "ARRAY":
            if word == '[':
                state = 'ЛВСКОБКА'
            else:
                raise SyntaxUnitError(f"Expected '[', but received '{word}' on state {state}")
        elif state == "ЛВСКОБКА":
            if word_class == 'знак':
                state = "ЗНАК_НИЖ"
                ...
                raise SyntaxUnitError(f"Expected 'знак' or 'целое', but received '{word}' on state {state}")
        elif state == "ЗНАК_НИЖ":
            if word_class == "целое":
                state = "ЦЕЛОЕ_НИЖ"
            else:
                raise SyntaxUnitError(f"Expected 'целое', but received '{word}' on state {state}")
        elif state == "ЦЕЛОЕ_НИЖ":
            if word_class == "бинзнак" or word == "div" or word == "mod":
                state = "БИНЗНАК_НИЖ"
            else:
                raise SyntaxUnitError(f"Expected 'бинзнак' or 'div' or 'mod', but received '{word}' on state {state}")
        elif state == "БИНЗНАК_НИЖ":
            if word_class == "идент" and identify_pascal(word) is False:
                state = "ИМЯ_НИЖ"
            else:
                raise IdUnitError(f"Invalid name of the first variable ({word}) on state {state}")
        elif state == "ИМЯ_НИЖ":
            if word == "..":
                state = "ДВЕТЧК"
            else:
            ...
            if word_class == "идент" and identify_pascal(word) is False:
                ...
                raise IdUnitError(f"Invalid name of the second variable ({word}) on state {state}")
        elif state == "ИМЯ_ВЕРХ":
            if word == "]":
                state = "ПРСКОБКА"
            else:
            ...
            if word == "of":
                ...
                raise SyntaxUnitError(f"Expected 'of', but received '{word}' on state {state}")
        elif state == "OF":
            if word_class == "идент" and identify_stdtype(word) is True:
                state = "STDTYPE"
            else:
                raise SyntaxUnitError(f"Expected 'stdtype', but received '{word}' on state {state}")
        elif state == "STDTYPE":
            if word == ";":
                state = "ТЧКЗПТ"
            else:
